        ...
        extends AbstractTestQueryFramework
{
    ...
    public void testCaseSensitiveMetricNames()
    {
        ...
        MaterializedResult lowerCaseResult = getQueryRunner().execute(getSession(), "SELECT value FROM \"http_requests_total\"");
        ...
        MaterializedResult upperCaseResult = getQueryRunner().execute(getSession(), "SELECT value FROM \"HTTP_REQUESTS_TOTAL\"");
        assertEquals(upperCaseResult.getOnlyColumnAsSet().iterator().next(), 20.0);

        // Test mixed case metric
        MaterializedResult mixedCaseResult = getQueryRunner().execute(getSession(), "SELECT value FROM \"HttpRequests_Total\"");
        assertEquals(mixedCaseResult.getOnlyColumnAsSet().iterator().next(), 30.0);

        // Test metrics with only one case version
        MaterializedResult apiCallsResult = getQueryRunner().execute(getSession(), "SELECT value FROM \"api_calls\"");
        ...
        assertQueryFails("SELECT * FROM \"API_CALLS\" WHERE value > 0", ".*Table prometheus.default.API_CALLS does not exist");

        MaterializedResult cpuUsageResult = getQueryRunner().execute(getSession(), "SELECT value FROM \"CPU_USAGE\"");
        ...
        assertQueryFails("SELECT * FROM \"cpu_usage\" WHERE value > 0", ".*Table prometheus.default.cpu_usage does not exist");
        ...
        mockClient.setTableOverride("UP", null);
        mockClient.setFetchUriOverride("UP", mockClient.createEmptyResponse());
        assertQueryFails("SELECT * FROM \"UP\" WHERE value > 0", ".*Table prometheus.default.UP does not exist");
    }
    @Test
    public void testMixedCaseLabels()
    {
        ...
        assertQuerySucceeds("SELECT labels FROM \"up\"");

        // Verify that we can see the labels in the output
        MaterializedResult result = getQueryRunner().execute(getSession(), "SELECT labels FROM \"up\"");
        ...
        assertQuerySucceeds("SELECT CAST(labels AS JSON) FROM \"up\"");
        assertQuerySucceeds("SELECT * FROM \"up\"");

        // Test count query
        MaterializedResult countResult = getQueryRunner().execute(getSession(), "SELECT count(*) FROM \"up\"");
            ...
            extends PrometheusClient
    {
        ...
        {
            super(new PrometheusConnectorConfig(), metricCodec, typeManager);
        }
