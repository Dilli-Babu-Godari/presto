...
/**
 * SQL Server-specific metadata implementation that adds view support.
 * This class extends JdbcMetadata to provide SQL Server-specific view operations
 * including listing views and retrieving view definitions from INFORMATION_SCHEMA.VIEWS.
 */
public class SqlServerMetadata
        extends JdbcMetadata
{
    private static final Logger LOG = Logger.get(SqlServerMetadata.class);
    ...
    public Map<SchemaTableName, ConnectorViewDefinition> getViews(ConnectorSession session, SchemaTablePrefix prefix)
    {
        ...
        if (prefix.getSchemaName() != null && prefix.getTableName() != null) {
            ...
            viewNames = sqlServerClient.listViews(session, identity, Optional.of(prefix.getSchemaName()));
        }
        else {
            // All views in all schemas
            viewNames = sqlServerClient.listSchemasForViews(session, identity);
        }
        ...
        return sqlServerClient.getViews(session, identity, viewNames);
    }
    ...
    {
        JdbcIdentity identity = JdbcIdentity.from(session);
        return sqlServerClient.listViews(session, identity, schemaName);
    }
    ...
    {
        JdbcIdentity identity = JdbcIdentity.from(session);
        sqlServerClient.createView(session, identity, viewMetadata.getTable(), viewData, replace);
    }
    ...
    {
        JdbcIdentity identity = JdbcIdentity.from(session);
        sqlServerClient.dropView(session, identity, viewName);
    }
}
